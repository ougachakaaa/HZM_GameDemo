...
public class WeaponWithCollider : MonoBehaviour
{
    ...
    public float CharacterAttackPoint
    {
        ...
        {
            if (_controller != null)
                return _playerLivingEntity.attackPoint;
            else
            {
    ...
    //fire
    [Header("Collider")]
    public WeaponCollider _collider;
    ...
    public void UpdateCollider(int count)
    {
        ...
        for(int i = 1; i <= count; i++)
        {
            ...
            _colliders.Add(Instantiate<WeaponCollider>(_collider, _spawnPos, Quaternion.identity, transform));
